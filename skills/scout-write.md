...
- **§5 (Vecenie 4-paragraph narrative)** — Identity → Strength → Weakness → Projection-verdict.
  - **§5.1 — narrative posture: essence, not snapshot.** Target 0–2 stats per paragraph, ~3–6 sentences each, prose-first. Surviving stats must be career-defining: championship/MVP counts, all-time records, career-span splits (`career 91.2%`), structural physical facts, two-season aggregates when the recent pattern is the essence. Paragraph evidence anchors on representative patterns (career-spanning tendencies, multi-year traits, recurring matchup motifs, career-credential classes) over snapshot moments (single-game, single-season possessions). Single-season percentages relocate to the sub-domain rationale row that scores that skill — the table is where snapshot-in-time evidence belongs. Per [PUBLIC-LANGUAGE-GUIDE §5.1](../docs/PUBLIC-LANGUAGE-GUIDE.md).
  ...
  - §5.4 — calibration target. The Curry healthy-vet sample and the Jackson dormant-vet Projection-verdict variant at PUBLIC-LANGUAGE-GUIDE §5.4 are the system-output targets (both post-pivot canon). Voice exemplars in PUBLIC-VOICE-CALIBRATION (Vecenie / Lowe / Schmitz) are reference inspiration only — match the §5.4 target, NOT the upstream writers.
...
**R5 — Tyler approval is the canonical voice gate.** Voice is judged by Tyler at scout-publish Phase B, not by scout-write or scout-review. scout-write's job is a quality first draft that minimizes Tyler's edit burden; Tyler's edit pass at Phase B is canonical.

**R6 — Calibration target: §5.4 Curry (healthy-vet) and Jackson (dormant-vet) samples.** Upstream voice exemplars (Vecenie, Lowe, Schmitz) in PUBLIC-VOICE-CALIBRATION are reference inspiration only. Both [PUBLIC-LANGUAGE-GUIDE §5.4](../docs/PUBLIC-LANGUAGE-GUIDE.md) samples are post-pivot canon — 0–2 stats per paragraph, career-defining, essence-not-snapshot — and are the system-output target. Match the Curry sample for healthy-vet narratives; match the Jackson dormant-vet Projection-verdict variant when profile §9 `Tense: past` or `Tense: mixed` applies (Identity / Strength / Weakness still follow the Curry template).

---

*Sub-skill of scout-publish. Loads PUBLIC-LANGUAGE-GUIDE + PUBLIC-VOICE-CALIBRATION + SIGNATURES + PUBLIC_MD_TEMPLATE in fresh editorial context; returns the full `_public.md`-shape draft text.*
