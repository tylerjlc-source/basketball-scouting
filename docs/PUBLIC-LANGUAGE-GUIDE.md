...
**Narrative posture: essence, not snapshot.** The four paragraphs speak to the player writ large — career identity, career strengths, career limitations, career verdict. The 26 sub-domain rationale table and 8 domain one-lines carry the snapshot-in-time evidence; the narrative carries the synthesis. Target stat density: **0–2 numbers per paragraph**, prose-first. Each surviving number must be career-defining — championship/MVP/All-NBA counts, all-time records, career-span splits ("career 91.2% from the line"), structural physical facts (height, wingspan, vertical), or two-season raw aggregates when a recent-window pattern is the essence (dormant-vet baseline, post-injury return, breakout-year emergence). Single-season percentages anchored to one recent year are stripped from narrative and **relocated to the sub-domain rationale row that scores that skill** — that is where snapshot-in-time evidence belongs. Identity sections in the existing published profiles already match this register (career credentials + structural profile). Strength sections are the structural break and require the largest rewrite under the pivot. Codifies the 2026-05-10 narrative-essence pivot (see footer); applies only to the 4-paragraph narrative — §4-B sub-domain rationales, §7 domain one-lines, §4-B Projection prose, §9 Career stats appendix are unchanged.
...
- **Reconcilable temporal frame on every narrative number.** Every percentage and granular split in narrative + sub-domain rationale carries a temporal frame the reader can reconstruct against visible data. Three legitimate forms:
  ...
  Default emphasis **in the 4-paragraph narrative**: career-aggregate row when the trajectory is the point ("career 91.2%"), or two-season raw aggregate when a recent pattern is the essence ("39.6% on 251 of 623 since 2024-25"). Single-named-season form ("40.7% in 2025-26") is permitted in narrative only for **very significant seasons** — MVP / Finals MVP year, championship year, breakout year, dormant-vet return baseline. Default emphasis **in the sub-domain rationale table and domain one-lines**: most-recent season or two-season aggregate per the prior policy — two-season aggregate is preferred for advanced/sample-sensitive stats (DFGPOE, drives, contested-shot splits) where current-season alone is too thin. The narrative tells the reader who the player is across his career; the structured fields show the reader what he did most recently. Aggregate ≈ weighted within ~0.5% in typical cases — well inside R14 dial tolerance — so the reader sees a defensible reconcilable number that matches what the score is calibrated against.
